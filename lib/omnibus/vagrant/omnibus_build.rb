...
class OmnibusStage < ::Vagrant::Command::Base

  def execute
    opts = OptionParser.new do |opts|
      opts.banner = "Usage: vagrant omnibus stage <vm-name>"
    end
...
class OmnibusBuild < ::Vagrant::Command::Base
  ...
  def execute
    ...
    # If building Private Chef add the chef-server-base recipe to
    # speed up compilation
    if argv[1] == "private-chef"
      @env.config.global.vm.provisioners[0].config.add_recipe("chef-server-base")
    end
